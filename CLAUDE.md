...
- ◐ Collider Lab (Task 13, **Phases 1–2 shipped**) — dedicated `Collider Lab` tab: grid-backed ground-truth inspector that reveals out-of-bounds colliders, **plus in-place polygon editing** (drag/add/remove vertices, pull-in-bounds, save) and a **single-item zip download** (SVG + manifest entry) for quick gist sim tests. Remaining Phase-2 sub-items (numeric vertex table, "⚠ N out-of-bounds" filter) + polygon generation + pill editor are ahead. See [Collider Lab](#collider-lab)
...
> **Direction note (partly shipped):** the *generator* is still **convex-only today**, but the downstream gist repo's concave-collider refactor (Phase 0 SHIPPED there) means this repo gains a **concave outer-boundary outline** path — emit the true concave silhouette labeled `type:"convex"` (gist decomposes it into a `compound` at load via `poly-decomp`; we do NOT add that dependency here). **Already shipped (2026-06-25, Task 12 change-set #2):** the validation layer no longer treats concave as forbidden. `validateConvex` now enforces the 8-vertex cap **only for genuinely-convex polygons** (the shapes Planck consumes directly); a concave outline just has to be a **simple (non-self-intersecting) closed ring** — the per-part ≤8 limit is gist's to enforce *after* decomposition, since this repo can't run `poly-decomp` to compute it. New `isSimplePolygon()` helper does the self-intersection test. The **Collider Lab editor** already shows "concave → decomposed downstream (allowed)"; the **DetailModal `ColliderEditor` warning is still the old "engine will use the convex hull" message** (wrong — Task 12 change-set #3, not yet flipped). **Still ahead:** the generator's ordered outer-boundary extraction (it currently hulls an unordered point cloud; chosen approach is vector polygon union, not bitmap tracing). Note: the 8-vertex cap exists **solely for Planck** — matter.js was removed from gist 2026-05-11, and Rapier re-hulls each part with no limit. See **Dev_Tasks.md → Task 12** and [../gist/Notes_on_Concave_Colliders_Refactor.md](../gist/Notes_on_Concave_Colliders_Refactor.md).

### Collider Lab

A dedicated **`Collider Lab`** tab (third tab in [TabStrip.jsx](src/components/TabStrip.jsx), alongside SVG Manager + Data Transforms) that pulls collider review out of the cramped DetailModal into a spatial audit/triage surface. **Task 13 — Phases 1–2 shipped** (audit/triage + polygon editing + single-item download). Remaining Phase-2 sub-items (numeric vertex table, "⚠ N out-of-bounds" filter) + polygon generation + pill editor are in [Dev_Tasks.md](Dev_Tasks.md).
...
- **"Edit collider"** (convex polygons only this phase) drops into a draft: drag vertices (via `getScreenCTM().inverse()`, works in any viewBox), click **+** to add at an edge midpoint, red **×** to remove, and **"Pull in-bounds"** to clamp all vertices into 0–W/0–H at once. The edit canvas is a **fixed generous space computed on entry** (viewBox ∪ collider bounds + margin) so the grid doesn't rescale mid-drag and every vertex stays reachable.
