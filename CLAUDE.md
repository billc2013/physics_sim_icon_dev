...
- ◐ Collider Lab (Task 13, **Phases 1–2 shipped, Phase 3 partial**) — dedicated `Collider Lab` tab: grid-backed ground-truth inspector that reveals out-of-bounds colliders, **in-place polygon editing** with a decluttered editor (color-state dots, keyboard delete, click-edge-to-add — 2026-07-02), pull-in-bounds, save, a **single-item zip download** (SVG + manifest entry) for quick gist sim tests, **two auto-fit outline-trace tools** (single-path + raster silhouette), and **exact Planck verdicts** (Task 15 — since 2026-07-17 `planckReadiness` runs pinned `poly-decomp@0.3.0` verdict-only, reporting real per-part vertex counts against the true cap of 12; forced by the runner/bird specimens). **Collider editing is now Lab-only** — DetailModal's collider section was removed 2026-07-02 (read-only overlay + "Edit in Collider Lab" link). Remaining Phase-2 sub-items (numeric vertex table, "⚠ N out-of-bounds" filter) + pill editor are ahead. See [Collider Lab](#collider-lab) and [DetailModal read-only collider](#detailmodal--read-only-collider-editing-moved-to-the-lab)
- ✓ Manifest v2 (Task 14 + the Task 12 `convex → polygon` rename, 2026-07-16) — entries carry `view_box: [x,y,w,h]` and emit collider type `"polygon"` at the export boundary; gist's loader half landed same day, dynamics_cart round-trip verified. See [Download approved](#download-approved-zip-export)
- ◐ Over-cap triage (Task 16, **2026-07-18 → 2026-08-06**) — **part 2 shipped, part 1 shipped except the compound/pill call**:
  ...
  - ◐ **Fix options (part 1):** ✓ **◯ Fit circle / ▭ Fit box** primitive tools + drag-handle **resize** (`PrimitiveEditLayer`) — the direct fix for round/rectangular ✖P items; circle/box are now first-class editable. ✓ **Auto-coarsen until Planck-safe + coarseness slider (2026-08-06)** — both trace tools now escalate their RDP epsilon until the exact Planck verdict passes (`coarsenUntilPlanckSafe` in [colliderGenerator.js](src/lib/colliderGenerator.js)), so a fresh trace is born Planck-safe instead of landing in the fix queue; a **Coarseness slider** in the ground-truth view re-simplifies the raw trace ring live for manual override. `bird` pilot re-traced + verified green 2026-08-06 (closed Task 15, incl. Bill confirming gist's dev-build CC2 warn). Part 1's only open thread: whether compound/pill authoring is still needed.
...
| `item.status` (enum)           | `physics_svgs.status`                      | draft / revised / approved / fix / idea_only |
...
- **Manifest (default on):** `manifest.json` emitted into the zip with shape `{ manifest_version: 2, exported_at, exported_by, export_mode, items: [{ name, display_name, status, version, color_tag, parent, view_box, physical_properties }] }`. The downstream physics-sim pipeline reads this to pick up metadata (mass, length, width) that's not encoded in the SVG itself. Bump `manifest_version` when the shape changes. **Each item entry is built by the shared `buildManifestEntry(item)` helper in [App.jsx](src/App.jsx)** — reused by both this zip export and the Collider Lab single-item download so the manifest shape (a gist-read contract) can't drift between the two. `physical_properties` uses `effectivePhysicalProperties` (inherited from parent for children); `parent` is the parent's name or null.
- **Manifest v2 (2026-07-16, Task 14 export half + Task 12 rename):** each entry carries `view_box: [x, y, w, h]` — the collider's authoring coordinate space, parsed from the item's own SVG via `parseViewBox` (`null` if unparseable → gist falls back to fetch-and-infer for that sprite). Lets gist skip its per-sprite viewBox fetch pass at sim mount (a mount-latency win on slow networks, not a correctness fix — gist's Option A already maps correctly). Additionally, the stored collider type `"convex"` is emitted as `"polygon"` (incl. inside `compound` parts) per the joint rename — **boundary-only**: DB rows, validation, the LLM contract, and all internal code still say `"convex"`; only `toManifestCollider()` in [App.jsx](src/App.jsx) translates at export.
...
**Programmatic fallback:** The client-side programmatic generator (`colliderGenerator.js`) extracts vertices from SVG elements, computes a convex hull, and simplifies to ≤12 vertices (`MAX_CONVEX_VERTICES`, the true Planck cap since 2026-07-17) — no LLM call. It **auto-runs on Import** (`handleImportAccept` in [App.jsx](src/App.jsx)) so a disk-imported SVG lands with a starter collider. Useful for SVGs that pre-date the LLM collider feature, or when the LLM's collider needs correction; refine the result in the Collider Lab.
...
- **Validation is convexity-aware** (mirrors `validateConvex`, keep in sync): a genuinely-convex polygon is hard-gated at ≤12 vertices (Planck eats it directly; cap corrected from 8 on 2026-07-17); a **concave outline is gated only on "simple closed ring"** (`isSimplePolygon`) with no outline vertex cap — its decomposed-part sizes are reported exactly by the Planck verdict (see Phase 3). Out-of-bounds **warns but allows save** (Bill's call); the amber overflow warning renders **below the canvas** so toggling it as a vertex crosses the edge never reflows the icon/grid/handles.
...
- Both emit `type:"convex"` with the raw ordered ring, **no hull**. Convex blobs (asteroid, balls) are still often better served by **◯ Fit circle / ≤12-hull** semantically, but since auto-coarsen (below) a trace on a convex blob no longer strands an over-cap polygon — it coarsens down to a Planck-safe ≤12-vertex ring.
- **Auto-coarsen until Planck-safe + coarseness slider (Task 16 part 1, shipped 2026-08-06).** Both trace tools end in "RDP an ordered ring", so Planck safety is largely a function of RDP epsilon (fine epsilon → smooth arcs decompose into many-vertex parts). `coarsenUntilPlanckSafe(ring)` in [colliderGenerator.js](src/lib/colliderGenerator.js) escalates epsilon (×1.4/pass, ≤12 passes, bail at ≤4 verts) re-running the **exact** `planckReadiness` verdict each pass until `ok` — it is the default inside BOTH `computeSilhouetteOutline` and `computeConcaveOutline` (`autoCoarsen:false` opts out), which now also return their raw `ring`. In the ground-truth view, while a trace draft is active, a **Coarseness slider** (between the Planck verdict and the edit toolbar) re-runs `ringToCollider(ring, ε)` live — the verdict recomputes as you slide (left = tighter fit, right = fewer verts). **Hand-editing a vertex or Pull-in-bounds detaches the slider** (the draft stops deriving from the ring, so re-sliding can't silently discard hand work); Fit circle/box, Edit collider, and Cancel clear it. The edit canvas is computed from the RAW ring's bounds so no slider position can push a vertex off-canvas. If even the coarsest pass fails (rare), the trace still stages the last attempt with a toast pointing at the slider/manual edit.
- **◯ Fit circle / ▭ Fit box** (`computeColliderForType`, added 2026-07-18 as a Task 16 part-1 fix option) — fit a Planck-safe **primitive** to the content (circle = centroid + max-distance bounding circle of the hull; box = axis-aligned bbox). Synchronous, mount-free (reuses `extractFilledVertices` + `convexHull`). This is the **fix for round/rectangular SVGs stuck as ✖P** — a primitive is always under the vertex cap, so it turns the verdict green. Like the trace tools, the result is **staged as a draft for review** (the overlay + Planck verdict update live) and written on Save via `onSaveCollider`.
- **Primitive resize (`PrimitiveEditLayer`, 2026-07-18).** Circle/box drafts are **interactively resizable**, not just fit-and-Save: a circle gets 4 cardinal radius handles + a center-move handle; a box gets 4 corner + 4 edge handles (opposite edge/corner anchored) + center-move. Same `getScreenCTM().inverse()` coordinate mapping as `PolygonEditLayer`; handles carry the pointer events, the shape outline doesn't. **Circle/box are now first-class `editable`** — the **"Edit collider"** button enters this resize editor on a *stored* circle/box (preserving its geometry), while **Fit circle/box** re-derive from content. Only `compound` stays read-only. The `EditToolbar` branches on `draft.type` (polygon → vertex-drag instructions + Pull-in-bounds; primitive → resize-handle instructions).

- **Planck-readiness verdicts (authoring-time, EXACT since 2026-07-17).** `planckReadiness(collider)` in [colliderSchema.js](src/lib/colliderSchema.js) returns `{ level: "ok" | "warn" | "fail", message }`. Concave outlines are **decomposed in place** with the same pinned `poly-decomp` calls gist runs at load (`makeCCW` + `quickDecomp`, mirroring gist's `decomposePolygonShape`), so the verdict reports **real per-part vertex counts**: circle/box/convex≤12 → `ok`; **convex >12 → `fail`** (decomposition can't reduce it); **concave, all parts ≤12 → `ok`** (with part facts, e.g. "39 verts → 10 parts, max 7"); **concave, any part >12 → `fail`** (with the offending parts, e.g. "part 10: 16 verts"); self-intersecting or decomposition-error → `warn`. The rewrite was forced by the **runner/bird specimens** (2026-07-16): both are "concave >8", yet runner decomposes safe and bird harmful — no outline-level threshold separates them. Surfaced as a colored **verdict line** in the ground-truth view (`PlanckVerdict`, recomputed live during edit) and **`⚠P`/`✖P` triage badges** in the Lab list ([ColliderLab.jsx](src/components/ColliderLab.jsx) `planckLevel`). Since auto-coarsen (2026-08-06) the trace buttons rarely emit a failing result; when even the coarsest pass fails, the toast points at the coarseness slider / manual edit / ◯▭ instead of the old "wrong tool" nudge.
- **Why verdict-only + still authoritative-in-gist:** `poly-decomp@0.3.0` is pinned to **gist's exact version** (the package is frozen upstream — latest since 2018 — so drift risk is nil). Decomposed parts are **discarded after the verdict** — never stored, never exported; gist remains the single source of decomposition for physics, and its dev-build post-decompose warn (gist roadmap CC2) is the engine-side backstop. The manifest carries **no** Planck field — gist already has the vertices, so stamping a heuristic would only duplicate + risk staleness.

> **We develop to Planck deliberately.** GIST keeps a **two-engine adapter (Planck + Rapier) with no default** — for the learning and to keep the adapter seam clean for future 3D / better-2D / cross-domain numerical engines. Planck is the strictest (12-vertex convex cap — `Settings.maxPolygonVertices`; **silently truncates to the first 12 verts then re-hulls**, distorting the fixture invisibly rather than crashing — the bird underbelly exhibit), so authoring to it means Rapier (which re-hulls every part) is safe for free. See **Dev_Tasks.md → Task 15**.
...
- **Self-intersection check has two known coverage gaps** (noted 2026-07-18, not yet fixed — no repro has bitten). `isSimplePolygon()` in [colliderSchema.js:243](src/lib/colliderSchema.js#L243) is the authoring-time gate that stops a self-intersecting concave outline from being saved (enforced in `validateConvex` [colliderSchema.js:121](src/lib/colliderSchema.js#L121), the live editor's save-block [ColliderGroundTruth.jsx:277](src/components/ColliderGroundTruth.jsx#L277), and the `⚠P` verdict). It's our proxy for what gist's `poly-decomp` will accept at load — so a case that passes here but fails there is the failure mode. Two gaps: **(1) coverage** — the check only runs on **concave `type:"convex"`** colliders (the concave branch of `validateConvex`). Convex polygons are simple by definition; circle/box are primitives; compound recurses per-part. So there is no self-intersection gate for a stored circle/box/compound — correct today, but a coverage boundary to remember. **(2) proper-crossings only** — `segmentsProperlyIntersect` uses strict sign difference (`d1 !== d2 && d3 !== d4`), so it catches clean crossings but **not degenerate touches**: a vertex lying exactly on a non-adjacent edge, or two collinear edges overlapping, both pass as "simple". Those can still choke gist's `poly-decomp` downstream. The trace tools rarely emit exact-collinear-touch rings, so this hasn't surfaced; if gist ever rejects a collider this repo passed, this is the first suspect. Promote to a `Dev_Tasks.md` task if it bites.
